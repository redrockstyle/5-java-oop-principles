 ...
 * */
public class Logger {
    private final ArrayList<MessageHandler> handles = new ArrayList<MessageHandler>();
    ...
    private void log(LoggerLevel lvl, String msg){
        ...
        if (lvl.ordinal() >= level.ordinal()) {
            for (MessageHandler handler : handles) {
                handler.handle(String.format("[" + lvl.toString() + "] " + dateFormat.format(new Date()) + " " + name + " - " + msg));
            }
        }
    }
